...
class Hls4mlOSG(BaseOSG):
    ...
    def write_makefile(self, ip_dir, project_name, reset=False, csim=True, synth=True,
        ...
        cmd_string = 'cd {}; {} -e "\n# Makefile automatically generated by hls4ml OSG\n\nall: {}\n\n'\
            .format(ip_dir, echo_cmd, project_name)
        cmd_string += '{project}:\n\tvivado_hls -f build_prj.tcl \"reset={reset} csim={csim} synth={synth} cosim={cosim} \
        validation={validation} export={export} vsynth={vsynth}\"\n\t@cat {project}_prj/solution1/syn/report/{project}_csynth.rpt'\
            .format(project=project_name, reset=reset, csim=csim, synth=synth, cosim=cosim, validation=validation, export=export, vsynth=vsynth)
        cmd_string += '\n\n" > Makefile'
